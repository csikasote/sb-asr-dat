...
class GRL(torch.nn.Module):
    def __init__(self, lambda_grl=0.01):
        super().__init__()
...
class ASR(sb.Brain):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Add DAT components
        self.grl = GRL(lambda_grl=0.01)  # From paper
        self.domain_classifier = DomainClassifier(input_size=hparams["dnn_neurons"])  # Match encoder output dim
    ...
    def compute_forward(self, batch, stage):
        ...
        # Domain Adversarial Network module
        device = next(self.domain_classifier.parameters()).device 
        grl_output = self.grl(encoded_signal).to(device)
        domain_pred = self.domain_classifier(grl_output.mean(dim=1))
        ...
        # Return domain_pred in predictions
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # Computing domain classification loss
        domain_pred = predictions["domain_pred"]
        device = domain_pred.device
        domain_labels = batch.domain.to(device)
        # If augmentations doubled the batch size, repeat labels
        if domain_labels.shape[0] != domain_pred.shape[0]:
            domain_labels = domain_labels.repeat(2)  # For 2x augmentation

        domain_labels = domain_labels.unsqueeze(1)  # Shape: (batch_size, 1)
        domain_loss = torch.nn.functional.binary_cross_entropy(
            domain_pred, domain_labels.float()
        )
        ...
        # Total loss = ASR loss + λ * domain_loss
        total_loss = loss + self.grl.lambda_grl * domain_loss
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.takes("wav", "domain")
    @sb.utils.data_pipeline.provides("sig", "domain_label")
    def audio_pipeline(wav, domain):
        """Load the audio signal. This is done on the CPU in the `collate_fn`."""
        sig = sb.dataio.dataio.read_audio(wav)
        yield sig
        yield torch.tensor(domain, dtype=torch.float32)

    # Define text processing pipeline. We start from the raw text and then
    # encode it using the tokenizer. The tokens with BOS are used for feeding
    # decoder during training, the tokens with EOS for computing the cost function.
    # The tokens without BOS or EOS is for computing CTC loss.
    @sb.utils.data_pipeline.takes("words", "domain")
    @sb.utils.data_pipeline.provides(
        "words", "tokens_list", "tokens_bos", "tokens_eos", "tokens", "domain_label"
    )
    def text_pipeline(words, domain):
        ...
        yield tokens
        yield torch.tensor(domain, dtype=torch.float32)  # Replicate domain label
    
    #@sb.utils.data_pipeline.takes("domain")
    #@sb.utils.data_pipeline.provides("domain_label")
    #def domain_pipeline(domain):
    #  yield torch.tensor(domain, dtype=torch.float32)
